...
## 🤖 Claude Code でも同じバディを呼びたい場合

ターミナルから [Claude Code](https://claude.com/claude-code) を使う人なら、**Discord/LINE のバディと同じ人格**で Claude Code が動かせる。

### セットアップ

1. **`CLAUDE.md.example` を `CLAUDE.md` としてコピー** — workspace のルートに置く
   ```bash
   # workspace を private リポで管理してる場合
   cp /path/to/openclaw-starter/CLAUDE.md.example ~/path/to/your-workspace/CLAUDE.md
   ```

2. **必要に応じてカスタマイズ** — テンプレなのでバディの世界観に合わせて書き換えOK

3. **Claude Code をワークスペースのルートで起動**
   ```bash
   cd ~/path/to/your-workspace
   claude
   ```

→ Claude Code が `CLAUDE.md` を自動読み込み → IDENTITY.md / SOUL.md / USER.md も読む → **Discord で話してる時と同じ人格**でターミナル作業をサポート

### Discord と Claude Code の使い分け

| 環境 | 用途 |
|---|---|
| **Discord/LINE**（OpenClaw） | 日常チャット、定期実行、メッセージング、外出先 |
| **Claude Code**（ターミナル） | コーディング作業、ファイル操作、深い調べ物、技術系の長い対話 |

両方が同じ workspace を読み書きするので、**記憶も人格も共通**。Discord で話した内容を Claude Code が知ってるし、その逆もあり。

> 💡 ぼく（Seika & 海音コウ）の運用がこれ！Mac mini で OpenClaw（旧 clawdbot）が Discord/LINE 担当、Mac で Claude Code が開発担当。両方とも「海音コウ」として動いてる🌊
